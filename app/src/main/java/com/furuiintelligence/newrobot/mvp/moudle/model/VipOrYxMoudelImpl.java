...
// ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
//          佛祖保佑     永不宕机      永无BUG
//        Name 赋睿智能-Date 2018/5/25-Time 8:37
//                   站在顶峰,看世界
//                   跌落谷底,思人生
...
public class VipOrYxMoudelImpl implements VipOrYxMoudel {
    @Override
    public void getVipOrYxMoudel(MyNetWorkCallback<VipOrYxEntity> callback, Map<String, String> vipMap) {
        iHttp.post(Url.VIP_YCJ, vipMap, callback);
    }
}
